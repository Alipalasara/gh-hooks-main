 ...
 * - after
 */
if ( ! class_exists( 'Exopite_Options_Framework' ) ) :

	class Exopite_Options_Framework {
		...
		protected function setup_multilang() {
			...
			$multilang_defaults = Exopite_Options_Framework_Helper::get_language_defaults();
		...
		public function is_special_multilang_active() {
			...
			return Exopite_Options_Framework_Helper::is_special_multilang_plugin_active();
		...
		protected function define_shared_hooks() {
			...
			Exopite_Options_Framework_Upload::add_hooks();
...
//				$this->config = wp_parse_args( $this->config, $default );
		...
		public function save( $posted_data ) {
			...
			$sanitizer = new Exopite_Options_Framework_Sanitize( $this->is_multilang(), $this->lang_current, $this->config, $this->fields );
			if ( $this->is_multilang() ) {
		...
		public function include_field_class( $field ) {
			...
			$class = 'Exopite_Options_Framework_Field_' . $field;
		...
		public function enqueue_field_class( $field ) {
			...
			$class = 'Exopite_Options_Framework_Field_' . $field['type'];
...
if ( ! function_exists( 'get_exopite_sof_option' ) ) {
	function get_exopite_sof_option( $option_slug, $default = false  ) {
		...
		if ( ! class_exists( 'Exopite_Options_Framework_Helper' ) ) {
			require_once 'multilang-class.php';
		}

		$language_defaults = Exopite_Options_Framework_Helper::get_language_defaults();
